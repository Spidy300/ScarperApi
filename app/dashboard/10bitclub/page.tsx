...
function MoviesGrid({ posts, searchQuery, isSearching }: { posts: TenBitClubPost[], searchQuery: string, isSearching: boolean }) {
  if (isSearching) {
    return (
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-5 lg:gap-6 mt-6">
      {posts.map((post, index) => {
        ...
        const id = urlParts[urlParts.length - 2] || urlParts[urlParts.length - 1] || index.toString();
        
        return (
          <a 
            key={index}
            href={`/dashboard/10bitclub/${id}`}
          ...
          >
            <div className="overflow-hidden flex flex-col">
              <div className="aspect-[2/3] relative rounded-lg overflow-hidden">
                <Image
                  src={post.imageUrl || 'https://placehold.jp/24/363636/ffffff/400x600.png?text=10BitClub'}
                  alt={post.title}
                  fill
                ...
                {/* {post.rating && (
                  <Badge className="absolute bottom-2 right-2 bg-amber-500 text-black text-xs">
                    ⭐ {post.rating}
                  </Badge>
                )} */}
              </div>
...
export default function TenBitClubDashboard() {
  const { user, loading: authLoading } = useAuth()
  const router = useRouter()
  const [movies, setMovies] = useState<TenBitClubPost[]>([])
  const [allMovies, setAllMovies] = useState<TenBitClubPost[]>([])
  ...
  const fetchMovies = useCallback(async (page: number = 1, search: string = "") => {
    ...
    try {
      ...
      const url = `/api/10bitclub${queryString ? `?${queryString}` : ''}`
  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/10bitclub?${params.toString()}`, {
        headers: {
          'x-api-key': userApiKey
        }
      })
            ...
            <CardContent className="pt-6">
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-6 max-w-sm mx-auto">
                  You need to create an API key first to access 10BitClub data.
                </p>
                <Button asChild>
