"use client"

import { useAuth } from "@/contexts/auth-context"
import { useRouter } from "next/navigation"
import { useEffect, useState, use } from "react"
import {
  Info,
  ExternalLink,
  Loader2,
  Copy,
  Check,
  Download,
  ArrowLeft,
  Calendar,
  Star,
  Tag
} from "lucide-react"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { Input } from "@/components/ui/input"
import { 
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle
} from "@/components/ui/dialog"
import { toast } from "sonner"
import Image from "next/image"

// Types for 10BitClub movie details
interface HubCloudLink {
  url: string
  text: string
}

interface HubCloudSection {
  title: string
  quality: string
  fileSize: string
  language: string
  hubCloudLinks: HubCloudLink[]
}

interface TenBitClubDetails {
  title: string
  posterImage?: string
  releaseDate?: string
  country?: string
  runtime?: string
  rating?: string
  synopsis: string
  hubCloudSections: HubCloudSection[]
  tags: string[]
  totalHubCloudLinks: number
  downloadSectionsCount: number
}

interface ApiResponse {
  success: boolean
  data?: TenBitClubDetails
  error?: string
}

export default function TenBitClubDetailPage({ params }: { params: { id: string } }) {
  const { user, loading: authLoading } = useAuth()
  const router = useRouter()
  const [movieDetails, setMovieDetails] = useState<TenBitClubDetails | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [showFullOverview, setShowFullOverview] = useState(false)
  const [selectedSection, setSelectedSection] = useState<any>(null)
  const [dialogOpen, setDialogOpen] = useState(false)
  const [hubcloudLinks, setHubcloudLinks] = useState<any[]>([])
  const [fetchingHubcloud, setFetchingHubcloud] = useState(false)
  const [copiedIndex, setCopiedIndex] = useState<number | null>(null)
  const [userApiKey, setUserApiKey] = useState<string | null>(null)

  // Unwrap the params object using React.use()
  const unwrappedParams = use(params);
  const { id } = unwrappedParams;
  
  useEffect(() => {
    if (!authLoading && !user) {
      router.push("/login")
    }
  }, [user, authLoading, router])

  // Fetch user's API key
  useEffect(() => {
    const fetchUserApiKey = async () => {
      if (!user) return;
      
      try {
        const response = await fetch(`/api/api-keys?userId=${user.uid}`);
        const data = await response.json();
        
        if (data.success && data.apiKeys && data.apiKeys.length > 0) {
          // Use the first active API key
          const activeKey = data.apiKeys.find((key: any) => key.isActive);
          if (activeKey) {
            setUserApiKey(activeKey.keyValue);
          }
        }
      } catch (error) {
        console.error('Failed to fetch user API key:', error);
      }
    };

    if (user) {
      fetchUserApiKey();
    }
  }, [user]);

  useEffect(() => {
    const fetchMovieDetails = async () => {
      if (!userApiKey) return;
      
      try {
        setLoading(true)
        // Reconstruct the full URL from the ID
        const fullUrl = `https://10bitclub.xyz/${id}/`
        
        const res = await fetch(`/api/10bitclub/details?url=${encodeURIComponent(fullUrl)}`, {
          headers: {
            'x-api-key': userApiKey
          }
        })
        const data: ApiResponse = await res.json()

        if (data.success && data.data) {
          setMovieDetails(data.data)
        } else {
          if (res.status === 401) {
            setError("API key required. Please create an API key in the API Keys section.")
          } else {
            setError(data.error || "Failed to fetch movie details")
          }
        }
      } catch (err) {
        setError("An error occurred while fetching movie details")
        console.error(err)
      } finally {
        setLoading(false)
      }
    }

    if (user && id && userApiKey) {
      fetchMovieDetails()
    }
  }, [user, id, userApiKey])

  const fetchHubCloudLinks = async (section: HubCloudSection) => {
    if (!userApiKey) {
      toast.error("API key required. Please create an API key in the API Keys section.")
      return
    }
    
    setFetchingHubcloud(true)
    setHubcloudLinks([])
    
    try {
      const allLinks = []
      
      // Process each HubCloud link in the section
      for (const link of section.hubCloudLinks) {
        try {
          const hubRes = await fetch(`/api/hubcloud?url=${encodeURIComponent(link.url)}`, {
            headers: {
              'x-api-key': userApiKey
            }
          })
          const hubData = await hubRes.json()
          
          if (hubData.success && hubData.links && hubData.links.length > 0) {
            // Add each stream URL as a separate entry
            hubData.links.forEach((streamLink: any) => {
              allLinks.push({
                url: streamLink.link || streamLink.url, // API returns 'link', not 'url'
                server: streamLink.server || 'Direct',
                type: streamLink.type || 'Unknown',
                isDirect: streamLink.isDirect || false,
                originalText: link.text,
                originalUrl: link.url,
                displayName: `${link.text} - ${streamLink.server || 'Direct'} (${streamLink.type || 'Unknown'})`,
                isStreamUrl: true
              })
            })
          } else {
            // If HubCloud fetch fails, still show the original link
            allLinks.push({
              url: link.url,
              displayName: `${link.text} (Failed to load)`,
              originalText: link.text,
              originalUrl: link.url,
              isStreamUrl: false
            })
          }
        } catch (error) {
          console.error("Error fetching HubCloud link:", error)
          allLinks.push({
            url: link.url,
            displayName: `${link.text} (Error loading)`,
            originalText: link.text,
            originalUrl: link.url,
            isStreamUrl: false
          })
        }
      }
      
      setHubcloudLinks(allLinks)
    } catch (error) {
      console.error("Error fetching HubCloud links:", error)
      toast.error("Failed to fetch HubCloud links")
      setHubcloudLinks([])
    } finally {
      setFetchingHubcloud(false)
    }
  }

  const handleDownloadClick = async (section: HubCloudSection) => {
    setSelectedSection(section)
    setDialogOpen(true)
    setCopiedIndex(null)
    fetchHubCloudLinks(section)
  }

  const isDirectDownloadLink = (url: string) => {
    if (!url || typeof url !== 'string') {
      return false;
    }
    return url.includes('video-downloads.googleusercontent.com') || 
           url.includes('drive.google.com') ||
           url.includes('direct-download')
  }

  const copyToClipboard = async (url: string, index: number) => {
    if (!url) {
      toast.error("No URL to copy");
      return;
    }
    
    try {
      await navigator.clipboard.writeText(url)
      setCopiedIndex(index)
      toast.success("URL copied to clipboard!")
      setTimeout(() => setCopiedIndex(null), 2000)
    } catch (error) {
      toast.error("Failed to copy URL")
    }
  }

  const handleDirectDownload = (url: string) => {
    if (!url) {
      toast.error("No download URL available");
      return;
    }
    // Open the download link in the same tab
    window.location.href = url
  }

  const goBack = () => {
    router.back()
  }

  if (authLoading) {
    return (
      <div className="flex min-h-svh items-center justify-center">
        <div>Loading...</div>
      </div>
    )
  }

  if (!user) {
    return null
  }

  return (
    <div className="flex flex-col min-h-screen">
      <div className="border-b">
        <div className="flex h-16 items-center px-4">
          <Button variant="ghost" size="icon" onClick={goBack} className="mr-2">
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <h1 className="text-lg font-semibold">10BitClub Movie Details</h1>
        </div>
      </div>
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0 overflow-y-auto">
        {loading ? (
          <div className="w-full mt-4">
            <div className="w-full h-64 bg-muted animate-pulse rounded-lg mb-4" />
            <div className="h-8 bg-muted animate-pulse rounded w-1/3 mb-2" />
            <div className="h-4 bg-muted animate-pulse rounded w-full mb-1" />
            <div className="h-4 bg-muted animate-pulse rounded w-5/6" />
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center h-[50vh] text-center">
            <p className="text-destructive mb-4">{error}</p>
            <Button onClick={() => window.location.reload()}>Retry</Button>
          </div>
        ) : movieDetails ? (
          <>
            {/* Movie Header Section */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {/* Movie Poster */}
              <div className="md:col-span-1">
                <div className="relative aspect-[2/3] rounded-lg overflow-hidden">
                  <Image
                    src={movieDetails.posterImage || 'https://placehold.jp/24/363636/ffffff/400x600.png?text=10BitClub'}
                    alt={movieDetails.title}
                    fill
                    className="object-cover"
                    sizes="(max-width: 768px) 100vw, 25vw"
                    priority
                  />
                </div>
              </div>
              
              {/* Movie Info */}
              <div className="md:col-span-3">
                <h1 className="text-2xl sm:text-3xl font-bold mb-4">{movieDetails.title}</h1>
                
                {/* Movie Meta Info */}
                <div className="flex flex-wrap gap-2 mb-4">
                  {movieDetails.releaseDate && (
                    <div className="flex items-center text-xs text-muted-foreground bg-white/5 px-2 py-1 rounded-full">
                      <Calendar className="h-3 w-3 mr-1" />
                      <span>{movieDetails.releaseDate}</span>
                    </div>
                  )}
                  {movieDetails.rating && (
                    <div className="flex items-center text-xs text-muted-foreground bg-amber-100/50 dark:bg-amber-400/10 px-2 py-1 rounded-full">
                      <Star className="h-3 w-3 mr-1 text-amber-500" />
                      <span>{movieDetails.rating}</span>
                    </div>
                  )}
                  {movieDetails.runtime && (
                    <div className="flex items-center text-xs text-muted-foreground bg-white/5 px-2 py-1 rounded-full">
                      <span>{movieDetails.runtime}</span>
                    </div>
                  )}
                  {movieDetails.country && (
                    <div className="flex items-center text-xs text-muted-foreground bg-white/5 px-2 py-1 rounded-full">
                      <span>{movieDetails.country}</span>
                    </div>
                  )}
                </div>

                {/* Tags */}
                {movieDetails.tags && movieDetails.tags.length > 0 && (
                  <div className="mb-4">
                    <h4 className="text-sm font-medium mb-2">Tags</h4>
                    <div className="flex flex-wrap gap-1.5">
                      {movieDetails.tags.map((tag, idx) => (
                        <Badge key={idx} variant="outline" className="text-xs">
                          <Tag className="h-3 w-3 mr-1" />
                          {tag}
                        </Badge>
                      ))}
                    </div>
                  </div>
                )}
                
                {/* Synopsis */}
                {movieDetails.synopsis && (
                  <div className="bg-slate-100 dark:bg-black/20 rounded-xl p-4 border border-slate-200 dark:border-white/5 mb-6">
                    <h2 className="flex items-center gap-2 text-lg font-semibold mb-3">
                      <Info className="h-4 w-4 text-purple-500" />
                      Synopsis
                    </h2>
                    <div className="relative">
                      <div className={`relative overflow-hidden transition-all duration-300 text-slate-700 dark:text-muted-foreground ${showFullOverview ? '' : 'max-h-[4.5em]'}`}>
                        <p>{movieDetails.synopsis}</p>
                      </div>
                      {!showFullOverview && movieDetails.synopsis.length > 200 && (
                        <div className="absolute bottom-0 left-0 w-full h-8 bg-gradient-to-t from-slate-100 dark:from-black/20 to-transparent"></div>
                      )}
                      {movieDetails.synopsis.length > 200 && (
                        <button 
                          onClick={() => setShowFullOverview(!showFullOverview)} 
                          className="text-purple-600 dark:text-purple-500 font-medium text-xs mt-2 hover:text-purple-500 dark:hover:text-purple-400 transition-colors"
                        >
                          {showFullOverview ? "Read Less" : "Read More"}
                        </button>
                      )}
                    </div>
                  </div>
                )}

                {/* Download Sections */}
                {movieDetails.hubCloudSections && movieDetails.hubCloudSections.length > 0 && (
                  <div className="mt-6">
                    <h3 className="text-lg font-semibold mb-3">
                      Download Links ({movieDetails.totalHubCloudLinks} HubCloud links available)
                    </h3>
                    <div className="space-y-3">
                      {movieDetails.hubCloudSections.map((section, idx) => (
                        <Button
                          key={idx}
                          variant="outline"
                          className="w-full text-left h-auto p-4 flex flex-col items-start gap-2"
                          onClick={() => handleDownloadClick(section)}
                        >
                          <div className="flex items-center gap-2 w-full">
                            <Download className="h-4 w-4 text-blue-500" />
                            <span className="font-medium text-sm">{section.quality}</span>
                            <Badge variant="secondary" className="text-xs">{section.language}</Badge>
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Size: {section.fileSize} • {section.hubCloudLinks.length} links
                          </div>
                          <div className="text-xs text-muted-foreground max-w-full truncate">
                            {section.title}
                          </div>
                        </Button>
                      ))}
                    </div>
                  </div>
                )}
              </div>
            </div>

            {/* Download Links Dialog */}
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>{selectedSection?.quality} Download</DialogTitle>
                  <DialogDescription>
                    Available HubCloud links and direct downloads
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  {fetchingHubcloud ? (
                    <div className="flex items-center justify-center py-8">
                      <div className="flex flex-col items-center gap-2">
                        <Loader2 className="h-8 w-8 animate-spin" />
                        <p className="text-sm text-muted-foreground">Fetching HubCloud links...</p>
                      </div>
                    </div>
                  ) : hubcloudLinks.length > 0 ? (
                    <div className="space-y-3">
                      <label className="text-sm font-medium">Available Download Links</label>
                      {hubcloudLinks.map((link, index) => (
                        <div key={index} className="space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-muted-foreground">
                              {link.displayName || 'Download Link'} {link.isStreamUrl ? '(Stream/Download)' : '(Link)'}
                            </span>
                          </div>
                          <div className="flex gap-2">
                            <Input
                              value={link.url || ''}
                              readOnly
                              className="flex-1 text-xs"
                            />
                            {link.url && isDirectDownloadLink(link.url) ? (
                              <Button
                                variant="default"
                                size="icon"
                                onClick={() => handleDirectDownload(link.url)}
                                className="shrink-0"
                              >
                                <Download className="h-4 w-4" />
                              </Button>
                            ) : (
                              <Button
                                variant="outline"
                                size="icon"
                                onClick={() => copyToClipboard(link.url || '', index)}
                                className="shrink-0"
                                disabled={!link.url}
                              >
                                {copiedIndex === index ? (
                                  <Check className="h-4 w-4 text-green-500" />
                                ) : (
                                  <Copy className="h-4 w-4" />
                                )}
                              </Button>
                            )}
                          </div>
                        </div>
                      ))}
                      <div className="pt-2 text-xs text-muted-foreground">
                        Stream links can be opened in VLC or your preferred media player. Download links can be used for file downloads.
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-8">
                      <p className="text-muted-foreground">No download links available</p>
                    </div>
                  )}
                </div>
              </DialogContent>
            </Dialog>
          </>
        ) : (
          <div className="flex items-center justify-center h-[50vh]">
            <p className="text-muted-foreground">No details found for this movie.</p>
          </div>
        )}
      </div>
    </div>
  )
}
